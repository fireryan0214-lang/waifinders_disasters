...
EARTHQUAKE_AUDIT = ROOT / "outputs" / "disaster_demo" / "worldwide_earthquake_pipeline_audit.json"
OUTPUT = ROOT / "materials" / "sentinel_live_operations.html"

data = json.loads(INPUT.read_text())
audit = json.loads(EARTHQUAKE_AUDIT.read_text()) if EARTHQUAKE_AUDIT.exists() else None
feed_health = data.get("feed_health", {"status": "DATA_FEED_UNAVAILABLE", "normal_operation_allowed": False, "operator_message": "Feed-health information is unavailable."})
health_colour = "#34d399" if feed_health.get("normal_operation_allowed") else "#f87171"
